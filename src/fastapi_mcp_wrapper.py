...
# ──────────────────────────────────────────────
# .env 자동 로드 (있을 때만 — Claude Desktop이 stdio로 기동할 때
# CWD 와 무관하게 wrapper와 같은 폴더의 .env 를 읽도록 강제)
# ──────────────────────────────────────────────
try:
    ...
    if _env_file.exists():
        ...
        sys.stderr.write(f"[subway-bim-fastapi] .env 로드: {_env_file}\n")
except ImportError:
    # python-dotenv 미설치는 비치명. 환경변수가 별도로 설정되어 있으면 그대로 동작
    pass
