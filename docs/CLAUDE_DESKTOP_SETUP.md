# Claude Desktop 이중 MCP 설정 가이드

> 원본: Phase 4-B 산출물. 경로·버전 표기는 현재 저장소 구조(`src/`)에 맞춰 갱신됨.
...
본 저장소의 `config/claude_desktop_config.example.json` 을 위 경로에 `claude_desktop_config.json` 이름으로 복사한 뒤,
다음 3개 값을 **실제 로컬 경로/키**로 수정한다.

| 치환 대상 | 예시 |
|-----------|------|
| `C:\\dev\\revit-mcp\\build\\index.js` | revit-mcp 저장소의 빌드된 index.js 절대경로 |
| `C:/dev/BIM/src/fastapi_mcp_wrapper.py` | 본 저장소의 `src/fastapi_mcp_wrapper.py` 절대경로 |
...
| subway-bim-fastapi 만 실패 | 터미널에서 직접 실행 검증: `python C:/dev/BIM/src/fastapi_mcp_wrapper.py` — ImportError 로그가 stderr 로 뜨면 `pip install -r requirements.txt` |
...
- `health_check`: `{ status: "ok", version: "2.0.0", spaces_loaded: [...5개...], validation_rules_count: 12 }`
...
| `list_validation_rules` | 검증 규칙 목록 (V001~V012 정의, V001~V008 판정 구현) |
